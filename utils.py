...
def observations_to_image_vla(observation: Dict, info: Dict) -> np.ndarray:
    r"""Generate image of single frame from observation and info
    returned from a single environment step().


    fyx: 这里进行了单个视频帧的图像拼接
    此函数为修改后的函数，只用于保存给vla进行训练的第一人称视频数据
    ...
    # 只使用指定的传感器
    sensor_name = 'agent_0_articulated_agent_jaw_rgb'
    ...
    assert (
        len(render_obs_images) > 0
    ), f"Expected sensor '{sensor_name}' to be enabled."

    # 由于只有一个传感器，直接使用该图像
    render_frame = render_obs_images[0]
